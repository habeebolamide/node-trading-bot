...
Yes — but only one per agent. Two different agents can both have active challenges simultaneously. Each carve-out is independent and additive (sum of all active `startingCapital` values is excluded from the main pool).
